...
- 一个用于从 web access.log 中统计指定日期（默认今天）响应码为 200 的 IP 出现次数，并列出前 N 个 IP 的小脚本。示例log文件：sample_access.log  
...
- 指定 topN 和日期（日期格式 `DD/Mon/YYYY`，例如 `10/Oct/2000`）：
  ./robust_top_ips.sh -f sample_access.log -n 10 -d 10/Oct/2000
...
bash -x ./robust_top_ips.sh -f access.log
```
- `PS4` 定制跟踪前缀，包含脚本名、行号和函数名，便于定位。

2. 使用 verbose 模式（脚本自带）：
```bash
